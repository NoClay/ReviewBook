GC是垃圾回收机制，java中将内存管理交给垃圾回收机制，这是因为在面向对象编程中一个对象的生命周期往往无法预料，所以我们无法为每个对象指定回收时机，但是我们可以采用`System.gc()`和`Runtime.getRuntime().gc()`进行请求垃圾回收，可以使用对象的`finalize()`对必要资源在垃圾回收之前进行处理。
